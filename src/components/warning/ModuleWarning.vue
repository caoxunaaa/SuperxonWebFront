<template>
  <div id="ModuleWarning">
    ...
    <div style="float:left;margin-left: 10px">
      <el-button @click="openDeployVoice" class="files" type="primary" style="padding: 12px 20px">修改告警责任人</el-button>
      <el-dialog
        title="配置告警责任人"
        :visible.sync="VoicedeployDialogVisible"
        width="1000px"
        append-to-body>
      ...
      <div style="width: 100%; height:50px;" id="content">
        ...
        <el-dialog
          ...
          <el-form ref="addform" :rules="addrules" :model="addform">
            ...
            <el-form-item>
              <el-button @click="add_form_button('addform')" style="background-color: #409eff; color: #fff; height: 40px; width:120px">新增</el-button>
            </el-form-item>
          </el-form>
        </el-dialog>
        <el-dialog
    ...
    <el-row :gutter="50" id="board">
      ...
      <el-col :span="12">
        <div class="grid-content bg-purple">
          <h3 style="background: white">正常品 告警</h3>
          <el-table
          :data="modulePnData"
          max-height="400"
          ...
          <h3 style="background: white">改制返工品 告警</h3>
          <el-table
      ...
      <el-col :span="12">
        <div class="grid-content bg-purple">
          <h3 style="background: white">正常品 工位告警</h3>
          <el-table
          :data="moduleNormalStationData"
          max-height="400"
          @row-click="onlyRowChart"
        ...
        <div class="grid-content bg-purple">
          <h3 style="background: white">改制返工品 工位告警</h3>
          <el-table
          :data="moduleReturnStationData"
          max-height="400"
          @row-click="onlyRowChart"
...
export default {
  ...
  methods: {
      ...
      that.$axios({
        ...
        if (res !== null) {
          ...
          for (let i = 0; i < that.correspondData.length; i++) {
            if (that.correspondData[i]['ModuleOsa'] === 'module') {
              console.log('模块')
    ...
    get_board_station_warning_data () {
      ...
      this.moduleBoardStationWarningData = Normal.concat(Return)
    ...
    // 获取正常品告警信息
    get_module_normal_pn (data) {
      let that = this
      const start = this.moment(new Date(new Date().setHours(0, 0, 0, 0))).format('YYYY-MM-DD HH:mm:ss')
      const end = this.moment(new Date(new Date().setHours(0, 0, 0, 0) + 24 * 60 * 60 * 1000)).format('YYYY-MM-DD HH:mm:ss')
      const work = '正常品'
      that.$axios({
        method: 'get',
        url: '/display/product-operation-info/module/warning-view/warning-info-in-time-period-by-work-order-type',
        params: {
          startTime: start,
          endTime: end,
          workOrderType: work
    ...
    get_module_return_pn (data) {
      ...
      that.$axios({
        ...
        if (res !== null && res.length > 0) {
          ...
          // console.log(that.returnmodulePnData)
        } else {
          that.returnmodulePnData = []
        }
      }).catch(function (err) {
    ...
    // 获取module_pn数据
    get_data (data) {
      this.get_module_normal_pn(data)
      this.get_module_return_pn(data)
      this.get_module_normal_station(data)
      this.get_module_return_station(data)
    },
    // 获取module_station工位数据
    // 正常品
    get_module_normal_station (data) {
      let that = this
      const start = this.moment(new Date(new Date().setHours(0, 0, 0, 0))).format('YYYY-MM-DD HH:mm:ss')
      const end = this.moment(new Date(new Date().setHours(0, 0, 0, 0) + 24 * 60 * 60 * 1000)).format('YYYY-MM-DD HH:mm:ss')
      const work = '正常品'
      that.$axios({
        method: 'get',
        url: '/display/product-operation-info/module/warning-view/warning-info-with-station-in-time-period-by-work-order-type',
        params: {
          startTime: start,
          endTime: end,
          workOrderType: work
    ...
    // 改制返工品
    get_module_return_station (data) {
      let that = this
      const start = this.moment(new Date(new Date().setHours(0, 0, 0, 0))).format('YYYY-MM-DD HH:mm:ss')
      const end = this.moment(new Date(new Date().setHours(0, 0, 0, 0) + 24 * 60 * 60 * 1000)).format('YYYY-MM-DD HH:mm:ss')
      const work = '改制返工品'
...
#ModuleWarning >>> .el-calendar-table .el-calendar-day {
  height: 50px;
}
#ModuleWarning >>> .el-calendar__body {
  padding: 0
}
</style>
