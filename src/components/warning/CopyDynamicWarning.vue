...
<div class="DynamicWarning" style="background-color: #ffffff;">
  <el-row>
    <el-col :span="6">
      <h3 style="margin: 0px">10G产线</h3>
      <el-table
        :data="station_10G_Data_Table_List"
        ...
        <el-table-column  :label="item.label" :property="item.property" :key="item.index" v-for="item in timelableData" width="7px">
    ...
    <el-col :span="6">
      <h3 style="margin: 0px">OSA产线</h3>
      <el-table
        :data="station_osa_Data_Table_List"
    ...
    <el-col :span="6">
      <h3 style="margin: 0px">其他产线</h3>
      <el-table
        :data="station_other_Data_Table_List"
        style="width: 100%;font-size:10px"
        :cell-style="FourcolumnStyle"
        border
        :header-cell-style="headerStyle">
        <el-table-column
        prop="station"
  ...
  methods: {
    ...
    get_data () {
      ...
      that.$axios({
        ...
        if (res !== null) {
          ...
          that.station_10G_Data_Table = that.get_table(res, that.station_10G_Data)
          that.station_10G_Data_Table_List = JSON.parse(JSON.stringify(that.onlyData))
          console.log(that.station_10G_Data_Table)
          // osa产线
          for (let z = 0; z < that.station_osa.length; z++) {
            for (let c = 0; c < station.length; c++) {
              // console.log(station[c].slice(0, 3))
              if (that.station_osa[z] === station[c].slice(0, 3)) {
                that.station_osa_Data.push(station[c])
                station.splice(c, 1)
                c--
              }
            }
          }
          that.station_osa_Data_Table = that.get_table(res, that.station_osa_Data)
          that.station_osa_Data_Table_List = JSON.parse(JSON.stringify(that.onlyData))
          console.log(that.station_osa_Data_Table)
          // olt产线
          ...
          // 25G产线
          that.station_other_Data = station
          that.station_other_Data_Table = that.get_table(res, that.station_other_Data)
          that.station_other_Data_Table_List = JSON.parse(JSON.stringify(that.onlyData))
          console.log(that.station_other_Data_Table)
        }
      })
    },
    ...
    OnecolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        ...
        if (this.station_10G_Data_Table[rowIndex][columnIndex - 1] === '0') {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (this.station_10G_Data_Table[rowIndex][columnIndex - 1] === '-1') {
    ...
    TwocolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        ...
        if (this.station_osa_Data_Table[rowIndex][columnIndex - 1] === '0') {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (this.station_osa_Data_Table[rowIndex][columnIndex - 1] === '-1') {
          return 'background-color: darkgray; color: darkgray'
        } else {
          return 'background-color: red; color: white;text-align: center;'
        }
    ...
    ThreecolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        ...
        if (this.station_olt_Data_Table[rowIndex][columnIndex - 1] === '0') {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (this.station_olt_Data_Table[rowIndex][columnIndex - 1] === '-1') {
          return 'background-color: darkgray; color: darkgray'
        } else {
          return 'background-color: red; color: white;text-align: center;'
        }
    ...
    FourcolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        return 'background:rgb(84, 92, 100);color:white;'
      } else {
        if (this.station_other_Data_Table[rowIndex][columnIndex - 1] === '0') {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (this.station_other_Data_Table[rowIndex][columnIndex - 1] === '-1') {
          return 'background-color: darkgray; color: darkgray'
        } else {
          return 'background-color: red; color: white;text-align: center;'
        }
