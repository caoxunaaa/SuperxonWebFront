...
<div class="DynamicWarning" style="background-color: #ffffff;">
  <el-row :gutter="20">
    <el-col :span="8">
      ...
      <h3 style="margin: 0px">10G产线 - 1</h3>
      <h5 style="margin: 0px;color: red;">(连续三支不良告警)</h5>
      <el-table
        :data="station_10G_Data_Table_List_one"
        style="width: 99%;font-size:10px"
        :cell-style="OnecolumnStyle"
        border
        ...
        <el-table-column  :label="item.label" :property="item.property" :key="item.index" v-for="item in timelableData" width="10px">
    ...
    <el-col :span="8">
      ...
      <h3 style="margin: 0px">10G产线 - 2</h3>
      <h5 style="margin: 0px;color: red;">(连续三支不良告警)</h5>
      <el-table
        :data="station_10G_Data_Table_List_two"
        style="width: 99%;font-size:10px"
        :cell-style="TwocolumnStyle"
        border
        :header-cell-style="headerStyle">
        <el-table-column
        prop="station"
    ...
    <el-col :span="8">
      ...
      <h3 style="margin: 0px">10G产线 - 3</h3>
      <h5 style="margin: 0px;color: red;">(连续三支不良告警)</h5>
      <el-table
        :data="station_10G_Data_Table_List_three"
        style="width: 99%;font-size:10px"
...
export default {
  ...
  methods: {
    ...
    get_data () {
      ...
      that.$axios({
        ...
        if (res !== null) {
          ...
          // console.log(station)
          // 10G产线
          for (let z = 0; z < that.station_10G.length; z++) {
            for (let c = 0; c < station.length; c++) {
              // console.log(station[c].slice(0, 3))
              if (that.station_10G[z] === station[c].slice(0, 3)) {
                that.station_10G_Data.push(station[c])
                station.splice(c, 1)
                c--
              }
            }
          ...
          // that.num = 3
          // that.spannum = parseInt(24 / that.num)
          // for (let u = 0; u < that.num; u++) {
          //   if (u === 2) {
          //     that.station_10G_Data_Table[u] = that.get_table(res, that.station_10G_Data.slice((40 * u), that.station_10G_Data.length))
          //   } else {
          //     that.station_10G_Data_Table[u] = that.get_table(res, that.station_10G_Data.slice((40 * u), (40 * (u + 1))))
          //   }
          //   that.station_10G_Data_Table_List[u] = JSON.parse(JSON.stringify(that.onlyData))
          //   console.log(that.station_10G_Data_Table[u])
          // }
          that.station_10G_Data_Table_one = that.get_table(res, that.station_10G_Data.slice(0, 40))
          that.station_10G_Data_Table_List_one = JSON.parse(JSON.stringify(that.onlyData))
          that.station_10G_Data_Table_two = that.get_table(res, that.station_10G_Data.slice(40, 80))
          that.station_10G_Data_Table_List_two = JSON.parse(JSON.stringify(that.onlyData))
          that.station_10G_Data_Table_three = that.get_table(res, that.station_10G_Data.slice(80, that.station_10G_Data.length))
          that.station_10G_Data_Table_List_three = JSON.parse(JSON.stringify(that.onlyData))
        }
        // console.log(that.station_10G_Data_Table_List[1])
      })
    },
    ...
    // 修改单元格颜色
    OnecolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        return 'background:rgb(84, 92, 100);color:white;'
      } else {
        if (Number(this.station_10G_Data_Table_one[rowIndex][columnIndex - 1]) === 0) {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (Number(this.station_10G_Data_Table_one[rowIndex][columnIndex - 1]) === -1) {
          return 'background-color: darkgray; color: darkgray'
        } else if (Number(this.station_10G_Data_Table_one[rowIndex][columnIndex - 1]) > 0) {
          if (Number(this.station_10G_Data_Table_one[rowIndex][columnIndex - 1]) < 10) {
            return 'background-color: rgb(255, 123, 123); color: white;text-align: center;'
          } else if (Number(this.station_10G_Data_Table_one[rowIndex][columnIndex - 1]) > 20) {
    ...
    TwocolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        ...
        if (Number(this.station_10G_Data_Table_two[rowIndex][columnIndex - 1]) === 0) {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (Number(this.station_10G_Data_Table_two[rowIndex][columnIndex - 1]) === -1) {
          return 'background-color: darkgray; color: darkgray'
        } else if (Number(this.station_10G_Data_Table_two[rowIndex][columnIndex - 1]) > 0) {
          if (Number(this.station_10G_Data_Table_two[rowIndex][columnIndex - 1]) < 10) {
            return 'background-color: rgb(255, 123, 123); color: white;text-align: center;'
          } else if (Number(this.station_10G_Data_Table_two[rowIndex][columnIndex - 1]) > 20) {
            return 'background-color: rgb(255, 0, 0); color: white;text-align: center;'
          } else {
            return 'background-color: rgb(255, 55, 55); color: white;text-align: center;'
          }
    ...
    ThreecolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        ...
        if (Number(this.station_10G_Data_Table_three[rowIndex][columnIndex - 1]) === 0) {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (Number(this.station_10G_Data_Table_three[rowIndex][columnIndex - 1]) === -1) {
          return 'background-color: darkgray; color: darkgray'
        } else if (Number(this.station_10G_Data_Table_three[rowIndex][columnIndex - 1]) > 0) {
          if (Number(this.station_10G_Data_Table_three[rowIndex][columnIndex - 1]) < 10) {
            return 'background-color: rgb(255, 123, 123); color: white;text-align: center;'
          } else if (Number(this.station_10G_Data_Table_three[rowIndex][columnIndex - 1]) > 20) {
            return 'background-color: rgb(255, 0, 0); color: white;text-align: center;'
          } else {
            return 'background-color: rgb(255, 55, 55); color: white;text-align: center;'
          }
  ...
  data () {
    return {
      ...
      spannum: 0,
      hypotheticalData: [],
      station_10G_Data_Table_one: [],
      station_10G_Data_Table_List_one: [],
      station_10G_Data_Table_two: [],
      station_10G_Data_Table_List_two: [],
      station_10G_Data_Table_three: [],
      station_10G_Data_Table_List_three: [],
