...
<script>
export default {
  name: 'DynamicWarningOtherstation',
  mounted () {
    this.get_data()
    this.setTimer()
  },
  methods: {
    ...
    FourcolumnStyle ({row, column, rowIndex, columnIndex}) {
      if (columnIndex === 0) {
        ...
        if (Number(this.station_other_Data_Table[rowIndex][columnIndex - 1]) === 0) {
          return 'background-color: #67C23A; color: #67C23A'
        } else if (Number(this.station_other_Data_Table[rowIndex][columnIndex - 1]) === -1) {
          return 'background-color: darkgray; color: darkgray'
        } else if (Number(this.station_other_Data_Table[rowIndex][columnIndex - 1]) > 0) {
          if (Number(this.station_other_Data_Table[rowIndex][columnIndex - 1]) < 10) {
            return 'background-color: rgb(255, 123, 123); color: white;text-align: center;'
          } else if (Number(this.station_other_Data_Table[rowIndex][columnIndex - 1]) > 20) {
