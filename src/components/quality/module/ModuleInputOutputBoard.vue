<template>
  <div class="ModuleInputOutputBoard">
    ...
    <el-table
      ...
      <el-table-column label="常温" align="center">
      <el-table-column
        prop="常温-投入"
        label="常温-投入"
      ></el-table-column>
      ...
      <el-table-column label="EE" align="center">
      <el-table-column
        prop="EE-投入"
        label="EE-投入"
      ></el-table-column>
...
export default {
  ...
  methods: {
    ...
    get_data () {
      ...
      that.$axios({
        method: 'get',
        url: '/display/product-statistic-query/input-and-output-summary',
        params: {
        ...
        if (res !== null) {
          ...
          for (let i = 0; i < that.data.length; i++) {
            ...
            that.allOneOneData += that.data[i]['调试-投入']
            that.allOneTwoData += that.data[i]['调试-产出']
            that.allOneThreeData += that.data[i]['调试-不良']
            that.allTwoOneData += that.data[i]['常温-投入']
            that.allTwoTwoData += that.data[i]['常温-产出']
            that.allTwoThreeData += that.data[i]['常温-不良']
            that.allThreeOneData += that.data[i]['EE-投入']
            that.allThreeTwoData += that.data[i]['EE-产出']
            that.allThreeThreeData += that.data[i]['EE-不良']
            that.allFourOneData += that.data[i]['入库-投入']
            that.allFourTwoData += that.data[i]['入库-产出']
            that.allFourThreeData += that.data[i]['入库-不良']
          }
          ...
          if (that.allFourThreeData !== 0) {
            that.allFourFourData = String(parseInt((that.allFourThreeData * 100) / that.allFourOneData)) + '%'
          } else {
            that.allFourFourData = '0%'
          }
          console.log(that.allOneFourData)
          that.data.push({'型号': '总计', '调试-投入': that.allOneOneData, '调试-产出': that.allOneTwoData, '调试-不良': that.allOneThreeData, '调试-不良占比': that.allOneFourData, '常温-投入': that.allTwoOneData, '常温-产出': that.allTwoTwoData, '常温-不良': that.allTwoThreeData, '常温-不良占比': that.allTwoFourData, 'EE-投入': that.allThreeOneData, 'EE-产出': that.allThreeTwoData, 'EE-不良': that.allThreeThreeData, 'EE-不良占比': that.allThreeFourData, '入库-投入': that.allFourOneData, '入库-产出': that.allFourTwoData, '入库-不良': that.allFourThreeData, '入库-不良占比': that.allFourFourData})
